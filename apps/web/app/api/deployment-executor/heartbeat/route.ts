import {parseDeploymentExecutorHeartbeatPayload} from '@fai-control-plane/application';
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const payload = parseDeploymentExecutorHeartbeatPayload(await readBoundedDeploymentExecutorJson(request, 1_024));
    if (leaseToken === null || payload === null) return denied();
    const result = await executor.service.heartbeat({authorization: executor.authorization, payload, leaseToken});
