import {parseRunnerCompletionPayload} from '@fai-control-plane/application';
import {
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const payload = parseRunnerCompletionPayload(
      await readBoundedRunnerJson(request, 16 * 1024)
    );
    if (leaseToken === null || payload === null) return denied();
    const completion = await runner.service.complete({
    ...
    return completion === null
      ? denied()
      : Response.json(completion, {status: 200, headers: noStore});
  } catch {
    return Response.json({status: 'unavailable'}, {status: 503, headers: noStore});
  }
}
