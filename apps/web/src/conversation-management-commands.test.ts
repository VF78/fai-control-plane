...
const authorization = {ok: true as const, session: {actorId, csrfToken: 'csrf'}, runtime: {config: {workspaceId}}, sessionToken: 'session'};
const request = (values: Record<string, string>) => new Request('https://app.example/projects/msa/chats', {
  method: 'POST', headers: {'content-type': 'application/x-www-form-urlencoded', referer: 'https://app.example/projects/msa/chats'},
