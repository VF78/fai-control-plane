...
import {CheckCircle2, Play, Save, Send, ShieldAlert, Sparkles} from 'lucide-react';
...
export function DeliveryProtocolEditor({projectId, protocol, csrfToken}: {
  ...
  return <section className="fcp-protocol-editor"><div className="fcp-protocol-facts"><span>Version {protocol.version}</span><span>Revision {protocol.revision}</span><span>{protocol.state}</span><span>{protocol.active ? 'Active' : 'Inactive'}</span></div><div className="fcp-protocol-table" role="table" aria-label="Delivery protocol stages"><div className="fcp-protocol-head" role="row"><span>Stage</span><span>Status</span><span>Responsibility</span><span>Mode</span><span>Evidence</span><span>Next</span></div>{definition?.stages.map((stage, index) => <div className="fcp-protocol-stage" role="row" key={stage.key}><label>Stage<input aria-label={`${stage.name} stage name`} disabled={!editable} value={stage.name} onChange={(event) => update(index, {name: event.target.value})}/><small>{stage.key}</small></label><label>Status<select aria-label={`${stage.name} task status`} disabled={!editable} value={stage.taskStatus} onChange={(event) => update(index, {taskStatus: event.target.value as typeof stage.taskStatus})}>{taskStatuses.map((status) => <option key={status}>{status}</option>)}</select><span className="fcp-check"><input aria-label={`${stage.name} enabled`} checked={stage.enabled} disabled={!editable} onChange={(event) => update(index, {enabled: event.target.checked})}/>Enabled</span></label><label>Responsibility<select aria-label={`${stage.name} responsibility`} disabled={!editable} value={roleOf(stage)} onChange={(event) => update(index, {responsibility: {kind: 'project_role', role: event.target.value as typeof roleOptions[number]}})}>{roleOptions.map((role) => <option key={role}>{role}</option>)}</select></label><label>Execution<select aria-label={`${stage.name} execution mode`} disabled={!editable} value={stage.executionMode} onChange={(event) => update(index, {executionMode: event.target.value as typeof stage.executionMode})}>{modes.map((mode) => <option key={mode}>{mode}</option>)}</select></label><label>Evidence<input aria-label={`${stage.name} required evidence`} disabled={!editable} value={stage.requiredEvidence.join(', ')} onChange={(event) => update(index, {requiredEvidence: event.target.value.split(',').map((item) => item.trim()).filter(Boolean)})}/></label><label>Next<select aria-label={`${stage.name} next stage`} disabled={!editable} value={stage.allowedNextStageKey ?? ''} onChange={(event) => update(index, {allowedNextStageKey: event.target.value || null})}><option value="">Complete</option>{definition.stages.slice(index + 1).map((item) => <option value={item.key} key={item.key}>{item.name}</option>)}</select></label></div>)}</div><div className="fcp-protocol-actions">{protocol.state === 'draft' ? <><button className="fcp-secondary" disabled={!editable || busy || !changed} onClick={() => void request('draft')}><Save aria-hidden="true" size={16}/>Save draft</button><button className="fcp-secondary" disabled={!editable || busy} onClick={() => void request('simulate')}><CheckCircle2 aria-hidden="true" size={16}/>Simulate</button><button className="fcp-primary-button" disabled={!editable || busy || simulation?.valid !== true} onClick={() => void request('publish')}><Send aria-hidden="true" size={16}/>Publish</button></> : protocol.state === 'published' && !protocol.active ? <button className="fcp-primary-button" disabled={csrfToken === null || busy} onClick={() => void request('activate')}><Play aria-hidden="true" size={16}/>Activate</button> : <p className="fcp-muted">Published versions are immutable. This is the active canonical version.</p>}</div>{changed ? <p className="fcp-diff"><strong>Before → after</strong> {definition?.stages.filter((stage, index) => !same(stage, original?.stages[index])).map((stage) => stage.name).join(', ') || 'No stage change'}</p> : null}{simulation === undefined ? null : <p className={`fcp-command-notice ${simulation.valid ? 'success' : 'error'}`}>{simulation.valid ? `Simulation valid · ${simulation.simulationHash.slice(0, 12)}` : simulation.violations.join(' ')}</p>}{notice === null ? null : <p className={`fcp-command-notice ${notice.tone}`}>{notice.text}</p>}</section>;
}

export function DeliveryJourneyAction({workItemId, taskVersion, journey, activeProtocolId, csrfToken}: {workItemId: string; taskVersion: number; activeProtocolId: string | null; csrfToken: string | null; journey: {version: number; deadlineAt: Date | null; protocolId: string; protocolVersion: number; stageKey: string} | null;}) {
  ...
  return <div className="fcp-journey-action">{journey === null ? <button className="fcp-primary-button" disabled={busy} onClick={() => void submit()}><Play aria-hidden="true" size={16}/>Start delivery journey</button> : <><label>Evidence references<textarea aria-label="Evidence references" value={evidence} onChange={(event) => setEvidence(event.target.value)} placeholder="Requirement: canonical evidence reference" /></label><button className="fcp-primary-button" disabled={busy} onClick={() => void submit()}><Send aria-hidden="true" size={16}/>Advance if allowed</button></>}{notice === null ? null : <p className={`fcp-command-notice ${notice.tone}`}>{notice.text}</p>}</div>;
}
