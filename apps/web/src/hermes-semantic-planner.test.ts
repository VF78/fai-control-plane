...
import {createHermesSemanticPlanner, hermesSemanticPlannerLimits, hermesSemanticPlanningConfiguration} from './hermes-semantic-planner';
...
const planningContext = {schemaVersion: 1 as const, projectId: artifact.projectId, deliveryProtocol: {id: '10000000-0000-4000-8000-000000000003', revision: 1, contentHash: 'b'.repeat(64), definition: {schemaVersion: 1 as const, stages: [{key: 'delivery', name: 'Delivery', enabled: true, taskStatus: 'in_dev' as const, responsibility: {kind: 'project_role' as const, role: 'contributor' as const}, executionMode: 'manual' as const, entryCriteria: ['Plan approved'], requiredEvidence: ['Change'], allowedNextStageKey: null}]}}, responsibilityCandidates: [{kind: 'project_role' as const, role: 'project_owner' as const}]};
const request = {idempotencyKey: 'project_plan.draft.generate.v1:plan:0:manifest', sourceManifest: [{artifactId: artifact.id, version: 1, sha256: artifact.sha256}], artifacts: [artifact], planningContext, planningContextHash: hashSemanticProjectPlanningContext(planningContext)};
const configured = {HERMES_SEMANTIC_PLANNING_ENABLED: 'true', HERMES_SEMANTIC_PLANNING_SOCKET: hermesSemanticPlannerLimits.socketPath, HERMES_SEMANTIC_PLANNING_TOKEN_FILE: hermesSemanticPlannerLimits.tokenFile};
...
it('sends the exact bounded project context through only the canonical authenticated UDS', async () => {
  ...
  expect(body).toEqual({schemaVersion: 1, operation: 'project_plan.draft.generate', idempotencyKey: request.idempotencyKey,
    sourceManifest: request.sourceManifest, planningContextHash: request.planningContextHash,
