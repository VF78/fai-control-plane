...
export function HermesContextControl({projectId, canManage, context}: Readonly<{
  ...
  return <section className="fcp-context-card"><header><div><strong>КОНТЕКСТ HERMES</strong><p>Канонические источники Control Plane применяются к новым задачам.</p></div><span className={`fcp-context-state ${tone}`}><i aria-hidden="true"/>{status}</span></header><dl className="fcp-context-facts"><div><dt>СОСТОЯНИЕ</dt><dd>{snapshot === null ? 'Не настроен' : 'Готов к использованию'}<small>{context?.status === 'stale' ? 'требуется обновление' : 'активен для Hermes'}</small></dd></div><div><dt>ПОСЛЕДНЕЕ ОБНОВЛЕНИЕ</dt><dd>{snapshot === null ? 'Не настроено' : contextTime(snapshot.createdAt)}<small>МСК</small></dd></div><div><dt>ИСТОЧНИКОВ</dt><dd>{context?.sources.length ?? 0}<small>канонические</small></dd></div><div><dt>ОБЪЁМ</dt><dd>{size === 0 ? 'Нет данных' : 'В пределах лимита'}<small>проверен при сборке</small></dd></div></dl><div className="fcp-context-sources-head"><span>Канонические источники</span><span>Доставка: Hermes</span></div><ul className="fcp-context-source-list" aria-label="Канонические источники контекста">{context?.sources.map((source) => <li key={source.key}><div><strong>{contextSourceLabel[source.key] ?? 'Источник проекта'}</strong><small>Подтверждённый источник</small></div><span>Источник</span></li>) ?? <li><div><strong>Источники не настроены</strong><small>Сборка недоступна</small></div></li>}</ul><div className="fcp-context-application"><span>Применение контекста</span><p>Новые задачи получают контекст сразу. В активном чате он применяется со следующего сообщения.</p></div><div className="fcp-context-action"><p>{canManage ? 'Владелец или оператор проекта может запустить сборку.' : 'Актуализация доступна владельцу или оператору проекта.'}</p>{canManage ? <AsyncButton type="button" pending={command.pending} pendingLabel="Актуализируем…" onClick={refresh}>Актуализировать контекст</AsyncButton> : null}</div><CommandNoticeView notice={command.notice}/></section>;
}
...
export function TaskExecutorControl({projectId, task, currentExecutor, confirmedRun}: Readonly<{projectId: string; currentExecutor: string; confirmedRun: ConfirmedRun|null; task: {itemId: string; status: string|null; blocked: boolean|null}}>) {
  ...
  const [runStatus, setRunStatus] = useState<'started'|'completed'|'failed'|'unknown'>(confirmedRun?.status ?? 'started');
  ...
  const refreshAttempt = () => { if (activeRun === null) return; void command.run(() => post('/api/tasks/executor', {action: 'refresh-attempt', projectId, projectItemId: task.itemId, deliveryReference: activeRun.deliveryReference}), {success: (result) => { const status = result.status as typeof runStatus; setRunStatus(status); return status === 'unknown' ? 'Статус запуска больше не подтверждается. Повтор требует вашего решения.' : status === 'started' ? 'Запуск всё ещё выполняется.' : status === 'failed' ? 'Запуск завершился ошибкой. Его можно запустить заново.' : 'Запуск завершён.'; }, error: 'Не удалось проверить статус запуска. Новая попытка не создана.'}); };
  const retryAttempt = (confirmUnobservableFailure: boolean) => { if (activeRun === null) return; void command.run(() => post('/api/tasks/executor', {projectId, projectItemId: task.itemId, executor: {kind: 'hermes'}, retry: {deliveryReference: activeRun.deliveryReference, nonce: crypto.randomUUID(), confirmUnobservableFailure}}), {success: 'Новая попытка Hermes запущена.', error: (error) => taskExecutorErrorNotice(error instanceof Error ? error.message : 'request_failed')}); };
  ...
  const runTitle = runStatus === 'completed' ? 'Запуск Hermes завершён' : runStatus === 'failed' ? 'Запуск Hermes завершился ошибкой' : runStatus === 'unknown' ? 'Статус запуска недоступен' : 'Запуск Hermes выполняется';
  const runAction = runStatus === 'started' ? <AsyncButton type="button" className="fcp-secondary" pending={command.pending} pendingLabel="Проверяем…" onClick={refreshAttempt}>Проверить статус</AsyncButton>
    : runStatus === 'failed' ? <AsyncButton type="button" pending={command.pending} pendingLabel="Запускаем…" onClick={() => retryAttempt(false)}>Запустить заново</AsyncButton>
      : runStatus === 'unknown' ? <AsyncButton type="button" pending={command.pending} pendingLabel="Запускаем…" onClick={() => retryAttempt(true)}>Запустить заново</AsyncButton> : null;
  return <section className="fcp-task-executor"><header><div><h2>Исполнитель</h2><p>GitHub остаётся источником назначения и статуса.</p></div><span>{currentExecutor}</span></header>{activeRun === null ? form : <><div className="fcp-task-outcome" role="status"><strong>{runTitle}</strong><p>{runStatus === 'unknown' ? 'Hermes больше не подтверждает состояние этого запуска.' : `Текущий этап: ${task.status ?? 'не подтверждён'}.`}</p><small>{new Intl.DateTimeFormat('ru-RU', {day:'numeric',month:'short',hour:'2-digit',minute:'2-digit',timeZone:'Europe/Moscow'}).format(new Date(activeRun.occurredAt))}</small><div>{runAction}</div></div><details className="fcp-task-reassign"><summary>{runStatus === 'completed' ? 'Начать текущий этап или сменить исполнителя' : 'Сменить исполнителя'}</summary>{form}</details></>}<CommandNoticeView notice={command.notice}/></section>;
}
