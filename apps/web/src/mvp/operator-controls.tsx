...
const useCommand = () => {
  ...
  return {notice, run: async (request: () => Promise<Result>) => { try { const result = await request(); setNotice(result.status ?? 'Сохранено'); } catch (error) { setNotice(error instanceof Error ? error.message : 'request_failed'); } }};
};
const id = () => globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random()}`;

export function CreateProjectControl() {
  const {notice, run} = useCommand();
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); void run(() => post('/api/projects', {slug: form.get('slug'), name: form.get('name'), repositoryUrl: form.get('repositoryUrl')})); };
  return <details className="fcp-control"><summary>Создать проект</summary><form onSubmit={submit}><label>Название<input name="name" required maxLength={200}/></label><label>Slug<input name="slug" required maxLength={100} pattern="[a-z0-9-]+"/></label><label>Repository URL<input name="repositoryUrl" required type="url"/></label><button className="fcp-primary">Создать</button></form><Notice value={notice}/></details>;
}
...
export function SourceAddControl({projectId}: Readonly<{projectId: string}>) {
  ...
  return <details className="fcp-control"><summary>Добавить источник</summary><form onSubmit={submit}><label>Название<input name="name" required maxLength={200}/></label><label>Тип<input name="kind" defaultValue="operator_note" required maxLength={64}/></label><label>Источник URL <input name="sourceUrl" type="url"/></label><label>Provenance<input name="provenance" defaultValue="operator" required maxLength={500}/></label><label>Содержание<textarea name="contentText" required maxLength={200000}/></label><button className="fcp-primary">Сохранить источник</button></form><Notice value={notice}/></details>;
}
...
export function ApprovalControl({projectId, taskId}: Readonly<{projectId: string; taskId: string | null}>) {
  ...
  return <details className="fcp-control"><summary>Зафиксировать согласование</summary><form onSubmit={submit}><label>Вид<select name="kind" defaultValue="acceptance"><option value="plan">plan</option><option value="internal_operation">internal operation</option><option value="acceptance">acceptance</option><option value="client_uat">client UAT</option></select></label><label>Решение<select name="decision"><option value="approved">approved</option><option value="rejected">rejected</option></select></label><button className="fcp-primary">Зафиксировать точную версию</button></form><Notice value={notice}/></details>;
}
...
export function AgentSubmitControl({projectId, tasks, sources}: Readonly<{
  ...
  const [notice, setNotice] = useState<string | null>(null); const [pending, setPending] = useState(false);
