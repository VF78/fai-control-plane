...
export function TaskExecutorControl({projectId, task, currentExecutor, confirmedRun}: Readonly<{projectId: string; currentExecutor: string; confirmedRun: ConfirmedRun|null; task: {itemId: string; status: string|null; blocked: boolean|null}}>) {
  ...
  const refreshAttempt = async () => { if (activeRun === null || pending) return; setPending(true); setNotice(null);
    try { const result = await post('/api/tasks/executor', {action: 'refresh-attempt', projectId, projectItemId: task.itemId,
      deliveryReference: activeRun.deliveryReference}); const status = result.status as typeof runStatus; setRunStatus(status);
      setNoticeTone(status === 'failed' ? 'error' : 'success');
      setNotice(status === 'unknown' ? 'Hermes больше не хранит статус этого запуска. Повтор возможен только после отдельного подтверждения завершения.' : status === 'started' ? 'Hermes подтверждает: запуск ещё выполняется.' : status === 'failed' ? 'Hermes подтвердил неуспешное завершение. Можно создать новую попытку.' : 'Hermes подтвердил завершение запуска.'); router.refresh();
    } catch { setNoticeTone('error'); setNotice('Hermes не подтвердил состояние запуска. Новая попытка не создана.'); } finally { setPending(false); }
  };
  const retryAttempt = async (confirmUnobservableFailure: boolean) => { if (activeRun === null || pending) return; setPending(true); setNotice(null);
  ...
  const runAction = runStatus === 'started' ? <button type="button" disabled={pending} onClick={() => void refreshAttempt()}>Проверить статус</button>
    : runStatus === 'failed' ? <button type="button" className="fcp-primary" disabled={pending} onClick={() => void retryAttempt(false)}>Подтвердить повтор</button>
      : runStatus === 'unknown' ? <button type="button" className="fcp-primary" disabled={pending} onClick={() => void retryAttempt(true)}>Подтвердить завершение и повтор</button> : null;
  return <section className="fcp-task-executor"><header><div><h2>Исполнитель</h2><p>GitHub остаётся источником назначения и статуса.</p></div><span>{currentExecutor}</span></header>{activeRun === null ? form : <><div className="fcp-task-outcome" role="status"><strong>{runTitle}</strong><p>{runStatus === 'unknown' ? 'Hermes больше не хранит этот статус; это не считается автоматическим отказом.' : `Текущий этап GitHub Project: ${task.status ?? 'не подтверждён'}.`}</p><small>Квитанция {activeRun.deliveryReference} · {new Intl.DateTimeFormat('ru-RU', {day:'numeric',month:'short',hour:'2-digit',minute:'2-digit',timeZone:'Europe/Moscow'}).format(new Date(activeRun.occurredAt))}</small><div>{runAction}</div></div><details className="fcp-task-reassign"><summary>{runStatus === 'completed' ? 'Начать текущий этап или сменить исполнителя' : 'Сменить исполнителя'}</summary>{form}</details></>}<Notice value={notice} tone={noticeTone}/></section>;
}
