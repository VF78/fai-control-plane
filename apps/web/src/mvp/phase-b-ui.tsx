...
import {AccessControls, ArchitectureProposalDecision, ProjectAgentActivationControl, ProjectDocumentUploadControl, ProjectRegistrationControl,
  SourceAddControl} from './operator-controls.tsx';
...
function Settings({project, sources, approvals, agentProfile}: Readonly<Pick<Props, 'project'|'sources'|'approvals'|'agentProfile'>>) {
  ...
  const otherSources=projectSources.filter(({kind})=>!kind.startsWith('project_document_v1:')&&
    kind!=='project_agent_profile_v1'&&!kind.startsWith('project_context_compact_v1:')&&
    kind!=='project_architecture_proposal_v1');
