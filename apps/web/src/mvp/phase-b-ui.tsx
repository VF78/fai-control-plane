...
const roleLabel:Record<string,string>={project_owner:'Владелец проекта',operator:'Оператор',contributor:'Участник',client:'Клиент'};
...
function Conversations({projects}: Readonly<{projects:readonly PhaseBProject[]}>) { return <div className="fcp-phase-b"><Header title="Чаты" detail="Рабочие каналы и доступ команды по каждому проекту."/>{projects.length===0?<EmptyProjects/>:<div className="fcp-portfolio-blocks">{projects.map(({project,evidence,config})=>{const telegramLinked=evidence?.people.filter((person)=>person.active&&person.identityBindings.some((binding)=>binding.provider==='telegram')).length??0;const bitrixLinked=evidence?.people.filter((person)=>person.active&&person.identityBindings.some((binding)=>binding.provider==='bitrix24')).length??0;return <ProjectBlock project={project} key={project.id}><div className="fcp-channel-grid"><article><MessageSquareText aria-hidden="true" size={19}/><div><strong>Внутренний чат</strong><small>Telegram · {telegramLinked} {participantNoun(telegramLinked)} с доступом</small></div><span className={`fcp-status ${config.telegram.configured?'success':'warning'}`}><CircleDot aria-hidden="true" size={13}/>{config.telegram.configured?'Готов':'Не настроен'}</span></article><article><MessageSquareText aria-hidden="true" size={19}/><div><strong>Чат с клиентом</strong><small>Bitrix24 · {bitrixLinked} {participantNoun(bitrixLinked)} с доступом</small></div><span className={`fcp-status ${config.bitrix.configured&&config.bitrix.clientActionsEnabled?'success':'neutral'}`}><CircleDot aria-hidden="true" size={13}/>{config.bitrix.configured&&config.bitrix.clientActionsEnabled?'Готов':'Недоступен'}</span></article></div></ProjectBlock>;})}</div>}</div>; }
