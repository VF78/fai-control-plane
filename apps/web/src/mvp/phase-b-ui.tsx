...
function Settings({project, sources, approvals}: Readonly<Pick<Props, 'project'|'sources'|'approvals'>>) {
  ...
  return <div className="fcp-phase-b"><Header title={project.name} detail="Факты проекта, источники Control Plane и точные согласования."/><section className="fcp-phase-b-binding"><header><div><h2>Подключение проекта</h2><p>{project.tracker.provider ?? 'Трекер не настроен'} остаётся единственным источником задач и статусов.</p></div><span><CircleDot aria-hidden="true" size={15}/>{project.tracker.freshness === 'fresh' ? 'Свежий Snapshot' : project.tracker.freshness === 'stale' ? 'Snapshot устарел' : 'Нет свежего snapshot'}</span></header><p>Только чтение. Редактирование привязки в этой рабочей области не поддерживается: показываются только факт подключения, источник и свежесть.</p><Facts columns={4}><Fact label="Репозиторий"><strong>{project.repositoryUrl.replace(/^https:\/\//, '')}</strong></Fact><Fact label="Трекер"><strong>{project.tracker.provider ?? 'Не настроен'}</strong></Fact><Fact label="Источник"><strong>{project.tracker.sourceUrl === null ? 'Нет ссылки провайдера' : 'Ссылка провайдера'}</strong></Fact><Fact label="Свежесть"><strong>{project.tracker.errorCode ?? project.tracker.freshness}</strong></Fact></Facts></section><section className="fcp-phase-b-manage"><header><div><h2>Подтверждённые источники</h2><p>Источники Control Plane</p></div><FileText aria-hidden="true" size={20}/></header><div className="fcp-phase-b-source-list">{projectSources.length === 0 ? <p className="fcp-empty">Источники ещё не добавлены.</p> : projectSources.map((source) => <article key={source.id}><div><strong>{source.name}</strong><small>{source.kind} · sha256 · {source.provenance}</small></div>{source.sourceUrl === null ? null : <a href={source.sourceUrl}><Link2 aria-hidden="true" size={14}/>Источник ↗</a>}</article>)}</div><SourceAddControl projectId={project.id}/></section><EvidenceList title="Точные согласования" rows={projectApprovals.map((approval) => ({id: approval.id, title: `${approval.kind} · ${approval.decision === 'approved' ? 'Согласовано' : 'Отклонено'}`, detail: `Точная наблюдённая версия · ${approval.targetVersion}`, at: approval.decidedAt, url: approval.targetUrl}))}/><p className="fcp-phase-b-note">Команда согласования доступна только после выбора свежей карточки в <a href={phaseHref('tasks', project.slug)}>«Задачах»</a>.</p></div>;
}
