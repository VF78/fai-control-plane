...
import type {ProjectAgentProfileView, ProjectHermesRuntimeSetupView, ProjectOperatorEvidenceView,
  ProjectSourceView, ProjectTaskView, ProjectTrackerCapabilities,ProjectTrackerPreparationView,ProjectWizardProgress} from '@fai-control-plane/db';
...
function Settings({projects,setup,actorId}: Readonly<{projects:readonly PhaseBProject[];setup:string|undefined;actorId:string}>) {
  ...
  return <div className={`fcp-phase-b fcp-settings ${setup!==undefined?'fcp-settings-wizard-open':''}`}><Header title="Настройки проектов"
    ...
    action={<Link className="fcp-primary fcp-add-project" href="/?view=settings&setup=new">Добавить проект</Link>}/>
    {setup!==undefined?<ProjectSetupWizard item={selected} contextCurrent={selectedContextCurrent} actorId={actorId}/>:null}
...
export function PhaseB(props:Props) { switch(props.view){case'conversations':return <Conversations projects={props.projects} actorId={props.actorId}/>;case'people':return <People projects={props.projects} actorId={props.actorId}/>;case'systems':return <Systems projects={props.projects}/>;case'settings':return <Settings projects={props.projects} setup={props.setup} actorId={props.actorId}/>;} }
