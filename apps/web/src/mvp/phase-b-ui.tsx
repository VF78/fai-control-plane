...
function SettingsProject(item: PhaseBProject) {
  ...
  const contextCurrent=agentProfile?.status==='ready'&&agentProfile.documentFingerprint===fingerprint;const setup=projectSetupState(item,documentsReady,contextCurrent);
...
function Settings({projects,setup,actorId,workspacePeople}: Readonly<{projects:readonly PhaseBProject[];setup:string|undefined;actorId:string;workspacePeople:readonly WorkspaceHumanActorView[]}>) {
  const ordered=[...projects].sort((left,right)=>{const state=(item:PhaseBProject)=>{const documents=projectDocumentState(item);return projectSetupState(item,documents.documentsReady,item.agentProfile?.status==='ready'&&item.agentProfile.documentFingerprint===documents.fingerprint).complete;};
    return Number(state(left))-Number(state(right));});
  const incomplete=ordered.filter((item)=>{const documents=projectDocumentState(item);return !projectSetupState(item,documents.documentsReady,item.agentProfile?.status==='ready'&&item.agentProfile.documentFingerprint===documents.fingerprint).complete;});
