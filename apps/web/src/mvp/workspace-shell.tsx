...
export function Shell({view,projects,projectCount,operatorName,children}: Readonly<{view?:PhaseArea;projects?:readonly ProjectTaskView[];projectCount?:number;operatorName:string;children:ReactNode}>) {
  ...
  const nav = (prefetch:boolean|null) => navigation.map((item) => { const Icon = icons[item]; return <WorkspaceLink href={phaseHref(item)} prefetch={prefetch} aria-current={active === item ? 'page' : undefined} key={item}><span className="fcp-nav-icon"><Icon aria-hidden="true" size={17}/></span><span>{labels[item]}</span></WorkspaceLink>; });
  return <div className="fcp-workspace"><a className="fcp-skip-link" href="#fcp-main">Перейти к содержимому</a><div className="fcp-shell-layout"><aside className="fcp-sidebar"><WorkspaceLink className="fcp-brand" href={phaseHref('dashboard')}><i aria-hidden="true">f</i><b>f(AI) Control</b></WorkspaceLink><nav className="fcp-sidebar-section fcp-sidebar-nav" aria-label="Разделы">{nav(null)}</nav></aside><header className="fcp-topbar"><WorkspaceLink className="fcp-mobile-brand" href={phaseHref('dashboard')}>f(AI) Control</WorkspaceLink><strong>{labels[active]}</strong><div className="fcp-topbar-actions"><span className="fcp-access-count"><ShieldCheck aria-hidden="true" size={15}/>{count} {projectNoun(count)}</span><span className="fcp-user-avatar" aria-label={`Оператор: ${operatorName}`}>{operatorName.split(/\s+/).map((part) => part[0]).join('').slice(0,2).toUpperCase()}</span><LogoutControl/></div><details className="fcp-mobile-menu"><summary aria-label="Открыть навигацию"><Menu aria-hidden="true" size={20}/></summary><div className="fcp-mobile-menu-body"><nav aria-label="Разделы">{nav(false)}</nav><LogoutControl/></div></details></header><main className="fcp-main" id="fcp-main" tabIndex={-1}><OfflineNotice/>{children}</main></div></div>;
}
