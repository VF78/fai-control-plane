...
export function Shell({view,projects,projectCount,operatorName,children}: Readonly<{view?:PhaseArea;projects?:readonly ProjectTaskView[];projectCount?:number;operatorName:string;children:ReactNode}>) {
  ...
  const nav = navigation.map((item) => { const Icon = icons[item]; return <WorkspaceLink href={phaseHref(item)} aria-current={active === item ? 'page' : undefined} key={item}><span className="fcp-nav-icon"><Icon aria-hidden="true" size={17}/></span><span>{labels[item]}</span></WorkspaceLink>; });
