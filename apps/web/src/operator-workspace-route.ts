...
export function workspaceRoute(path: readonly string[], query: WorkspaceQuery): WorkspaceRoute | null {
  ...
  const requestedHandoff = exact(query.handoff);
  const handoffResult = ['accepted', 'stale', 'forbidden', 'not_found', 'unavailable'].find((value) =>
    value === requestedHandoff) as WorkspaceRoute['handoffResult'];
