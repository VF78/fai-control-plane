...
import {CheckCircle2, FilePlus2, Save, ShieldCheck} from 'lucide-react';
import type {ProjectPlanDefinition, ProjectPlanSimulation} from '@fai-control-plane/domain';
...
export const postProjectPlan = async (body: Record<string, unknown>) => {
  ...
  const result = await response.json().catch(() => ({})) as {status?: string; message?: string; receipt?: unknown; simulation?: ProjectPlanSimulation};
...
export function ProjectPlanControls({projectId, plan, csrfToken, canEdit, canApprove}: Readonly<{projectId: string; plan: PlanData; csrfToken: string | null; canEdit: boolean; canApprove: boolean}>) {
  ...
  const run = async (operation: () => Promise<{receipt?: unknown; simulation?: ProjectPlanSimulation}>) => {
  ...
  return <div className="fcp-plan-workspace">
    ...
    <section className="fcp-plan-step"><header><span>3</span><div><h3>Утверждение Product Owner</h3><p>Утверждённая версия неизменяема и не создаёт scope baseline или задачи.</p></div></header>
