...
import {projectDossierReadiness, type ProjectPlanDefinition, type ProjectPlanMaterialization, type ProjectPlanSimulation, type ProjectSourceArtifactKind} from '@fai-control-plane/domain';
...
export function ProjectPlanControls({projectId, plan, csrfToken, canEdit, canApprove}: Readonly<{projectId: string; plan: PlanData; csrfToken: string | null; canEdit: boolean; canApprove: boolean}>) {
  ...
  return <div className="fcp-plan-workspace">
    ...
    <section className="fcp-plan-step"><header><span>1</span><div><h3>Исходные материалы</h3><p>Поддерживаются только вставленные text/plain, text/markdown и application/json; текст хранится в PostgreSQL с SHA-256.</p></div></header>
      ...
      <div className="fcp-plan-sources">{plan.artifacts.length === 0 ? <p className="fcp-empty-line">Источники ещё не добавлены.</p> : plan.artifacts.map((artifact) => <details key={artifact.id}><summary><strong>{artifact.name}</strong><span>{sourceKindLabel[artifact.sourceKind]} · {artifact.mediaType} · {artifact.sizeBytes} байт · {artifact.sha256.slice(0, 12)}</span></summary><pre>{artifact.content}</pre><small>ID для citation: <code>{artifact.id}</code> · {artifact.provenance.label}</small></details>)}</div>
