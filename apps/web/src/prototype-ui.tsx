...
function Access({route, project, access}: {route: PrototypeRoute; project: ProjectData; access: AccessData | null}) {
  ...
  const memberships = access.memberships.filter((membership) => membership.projectId === project.project.id);
  const actors = memberships.flatMap((membership) => access.actors.find((actor) => actor.id === membership.actorId) ?? []);
  ...
  const identities = selected === null ? [] : access.externalIdentities.filter((identity) => identity.actorId === selected.id);
  const grants = selected === null ? [] : access.resourceGrants.filter((grant) => grant.projectId === project.project.id && grant.actorId === selected.id);
  const profiles = selected === null ? [] : access.agentSystems.find((system) => system.actorId === selected.id)?.profiles ?? [];
...
function Agents({route, access, health}: {route: PrototypeRoute; access: AccessData | null; health: HealthData | null}) { const agents = access?.actors.filter((actor) => actor.type === 'agent') ?? []; const systems = new Map<string, AccessData['agentSystems'][number]>(access?.agentSystems.map((item) => [item.actorId, item] as const) ?? []); return <><div className="fcp-page-title"><div><h1>Agents &amp; Systems</h1><p>Persisted fleet, registrations, and operational evidence. Runtime liveness is Unknown unless a linked run records it.</p></div><Scope route={route}/></div><SystemsSummary health={health}/><section className="fcp-section"><div className="fcp-section-head"><h2>Agents</h2><span>Actor and profile records</span></div><div className="fcp-list">{agents.length === 0 ? <Blank title="Agents not observed">No agent records are available from PostgreSQL.</Blank> : agents.map((agent) => { const profiles = systems.get(agent.id)?.profiles ?? []; const registered = profiles.flatMap((profile) => profile.registrations); return <Link className="fcp-row fcp-agent-row" href={screenUrl({kind: 'agent', agentId: agent.id}, route.scope)} key={agent.id}><Bot aria-hidden="true" size={18}/><div><strong>{agent.displayName}</strong><small>{agent.role} · {profiles.length === 0 ? 'No profile observed' : `${profiles.length} persisted profile${profiles.length === 1 ? '' : 's'}`}</small></div><span>{registered.length === 0 ? 'Project registration not observed' : `${registered.length} project registration${registered.length === 1 ? '' : 's'} · runtime liveness Unknown`}</span><Status value={agent.disabledAt === null ? 'ready' : 'blocked'}/><ChevronRight aria-hidden="true" size={16}/></Link>; })}</div></section></>; }
function AgentDetail({route, access, csrfToken}: {route: PrototypeRoute; access: AccessData | null; csrfToken: string | null}) { const agent = access?.actors.find((item) => item.id === route.agentId && item.type === 'agent') ?? null; if (agent === null) return <Blank title="Agent not observed">This agent is not available in the canonical access read model.</Blank>; const profiles = access?.agentSystems.find((item) => item.actorId === agent.id)?.profiles ?? []; const hermes = access?.hermes?.actorId === agent.id ? access.hermes : null; return <><div className="fcp-page-title"><div><Crumbs route={route} project={null} title={agent.displayName}/><h1>{agent.displayName}</h1><p>{agent.role} · Runtime liveness: Unknown (not observed)</p></div><Status value={agent.disabledAt === null ? 'ready' : 'blocked'}/></div><div className="fcp-detail-layout"><main className="fcp-detail-main"><section className="fcp-section"><div className="fcp-section-head"><h2>Runtime registrations</h2><ServerCog aria-hidden="true" size={17}/></div>{profiles.length === 0 ? <p className="fcp-empty-line">No persisted agent profile or project registration is observed.</p> : profiles.map((profile) => <article className="fcp-agent-profile" key={profile.id}><header><strong>{profile.runtimeId} · {profile.runtimeProfile}</strong><span>Profile {profile.enabled ? 'enabled' : 'disabled'} · config {profile.configHash}</span></header><p>{profile.registrations.length === 0 ? 'Project scope not observed.' : profile.registrations.map((registration) => `${registration.project} (${registration.projectSlug}) · ${registration.provider}/${registration.runtimeKey} · ${registration.enabled ? 'enabled' : 'disabled'}`).join(' | ')}</p><p>Current run: {profile.latestRun === null ? 'Not observed' : `${profile.latestRun.status} · updated ${date(profile.latestRun.updatedAt)}`} · Last receipt: {profile.latestRun?.receipt === null || profile.latestRun?.receipt === undefined ? 'Not observed' : `${profile.latestRun.receipt.terminal} · ${date(profile.latestRun.receipt.completedAt)}`}</p></article>)}</section><section className="fcp-section"><div className="fcp-section-head"><h2>Effective instructions</h2><Bot aria-hidden="true" size={17}/></div>{profiles.length === 0 || profiles.every((profile) => profile.instruction === null) ? <p className="fcp-empty-line">Versioned effective instructions are not observed for this agent.</p> : profiles.map((profile) => profile.instruction === null ? null : <article className="fcp-agent-profile" key={`${profile.id}:instruction`}><strong>{profile.runtimeId} · {profile.runtimeProfile}</strong><p>{profile.instruction.provenance} · effective hash {profile.instruction.hash}</p></article>)}</section>{hermes === null ? null : <section className="fcp-section fcp-settings"><div className="fcp-section-head"><h2>Hermes profile</h2><span>Governed mutation</span></div>{csrfToken === null ? <p className="fcp-empty-line">An authenticated operator session is required.</p> : <form action="/api/agent-profiles/hermes" className="fcp-profile-form" method="post"><input name="_csrf" type="hidden" value={csrfToken}/><input name="expectedVersion" type="hidden" value={hermes.version}/><label>Instructions<textarea defaultValue={hermes.instructions} maxLength={2000} name="instructions" required rows={5}/></label><label>Evidence<select defaultValue={String(hermes.settings.includeEvidence)} name="includeEvidence"><option value="true">Required</option><option value="false">Optional</option></select></label><label>Status<select defaultValue={String(hermes.enabled)} name="enabled"><option value="true">Enabled</option><option value="false">Disabled</option></select></label><button className="fcp-primary-button" type="submit">Update profile</button></form>}</section>}</main><aside className="fcp-meta"><h2>Details</h2><DetailFacts items={[{label: 'Type', value: agent.type}, {label: 'Role', value: agent.role}, {label: 'Actor state', value: agent.disabledAt === null ? 'Enabled' : 'Disabled'}, {label: 'Runtime liveness', value: 'Unknown (not observed)'}, {label: 'Profiles', value: String(profiles.length)}]}/></aside></div></>; }
