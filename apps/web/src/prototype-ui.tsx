...
export type PrototypeRoute = Readonly<{
  screen: 'dashboard' | 'projects' | 'overview' | 'tasks' | 'task' | 'protocol' | 'runs' | 'run' | 'chats' | 'access' | 'agents' | 'agent';
...
function Scope({route}: {route: PrototypeRoute}) {
  ...
  return <div className="fcp-scope" aria-label="Current scope"><div><FolderKanban aria-hidden="true" size={15}/><span>Project</span><b>{route.project?.toUpperCase() ?? 'All projects'}</b></div><div><span>Environment</span><b>{scope.environment ?? 'Not configured'}</b></div><div><Clock3 aria-hidden="true" size={15}/><span>Time</span><b>{scope.from ?? scope.to ? 'Custom range' : 'All time'}</b></div></div>;
}
function Header({route}: {route: PrototypeRoute}) {
  const nav = [{label: 'Dashboard', icon: LayoutDashboard, target: {kind: 'dashboard'} as const}, {label: 'Projects', icon: FolderKanban, target: {kind: 'projects'} as const}, {label: 'Agents', icon: Bot, target: {kind: 'agents'} as const}];
  const current = route.screen === 'overview' || route.screen === 'tasks' || route.screen === 'task' || route.screen === 'protocol' || route.screen === 'runs' || route.screen === 'run' || route.screen === 'chats' || route.screen === 'access' ? 'Projects' : route.screen === 'agent' ? 'Agents' : route.screen[0]!.toUpperCase() + route.screen.slice(1);
  return <header className="fcp-global"><Link href={screenUrl({kind: 'dashboard'}, route.scope)} className="fcp-brand">f(AI)<span>Control</span></Link><nav className="fcp-global-nav" aria-label="Global navigation">{nav.map(({label, icon: Icon, target}) => <Link href={screenUrl(target, route.scope)} key={label} aria-current={current === label ? 'page' : undefined}><Icon aria-hidden="true" size={16}/><span>{label}</span></Link>)}</nav></header>;
}
...
function TaskDetail({route, project, runs}: {route: PrototypeRoute; project: ProjectData; runs: RunsData | null}) {
  ...
  return <><ProjectHeader route={route} project={project} title={task.title}/><div className="fcp-detail-layout"><main className="fcp-detail-main"><div className="fcp-detail-status"><Status value={task.blocked ? 'blocked' : task.status}/></div>{task.summary === null ? <p className="fcp-empty-line">Task summary not observed.</p> : <p className="fcp-task-summary">{task.summary}</p>}<section className="fcp-section"><div className="fcp-section-head"><h2>Task lifecycle</h2><span>{statusLabel(task.status)}</span></div><StageStrip items={['backlog', 'ready', 'in_dev', 'qa', 'acceptance', 'done']} current={task.status}/></section><section className="fcp-section"><div className="fcp-section-head"><h2>Run or packet handoff</h2><ListChecks aria-hidden="true" size={17}/></div><div className="fcp-command"><div><strong>{task.handoff?.label ?? 'Not observed'}</strong><p>{task.handoff === null ? 'No task packet or run handoff is recorded.' : `Observed ${task.handoff.kind} handoff.`}</p></div>{run === null ? null : <Link className="fcp-primary" href={runUrl(project.project.slug, run.id, route.scope)}>Open run receipt</Link>}</div></section><section className="fcp-section"><div className="fcp-section-head"><h2>Policy & approval</h2><ShieldCheck aria-hidden="true" size={17}/></div><p className="fcp-empty-line">{approval === null ? 'Not observed.' : `${statusLabel(approval.status)} · ${approval.environment}`}</p></section><section className="fcp-section"><div className="fcp-section-head"><h2>Evidence</h2><FileCheck2 aria-hidden="true" size={17}/></div><p className="fcp-empty-line">{run === null ? 'No linked run receipt observed.' : run.receipt === null ? 'Run receipt not observed.' : `Receipt recorded ${date(run.receipt.completedAt)}.`}</p></section></main><aside className="fcp-meta"><h2>Details</h2><DetailFacts items={[{label: 'Status', value: task.blocked ? 'Blocked' : statusLabel(task.status)}, {label: 'Responsible human', value: task.owner ?? 'Unknown'}, {label: 'Responsible agent', value: 'Unknown'}, {label: 'Approval', value: approval === null ? 'Not observed' : statusLabel(approval.status)}, {label: 'Source', value: task.externalUrl === null ? 'Not observed' : 'Provider observation'}, {label: 'Updated', value: date(task.updatedAt)}]}/>{task.externalUrl === null ? null : <a href={task.externalUrl} target="_blank" rel="noreferrer">Open provider source</a>}</aside></div></>;
}
function Protocol({route, project}: {route: PrototypeRoute; project: ProjectData}) {
  return <><ProjectHeader route={route} project={project}/><section className="fcp-settings"><div className="fcp-settings-head"><div><h2>Delivery protocol</h2><p>Configured version and responsibility are not observed.</p></div><Status value="unknown"/></div><p className="fcp-empty-line">No protocol record is available.</p></section></>;
}
...
function ProjectScreen({route, data}: {route: PrototypeRoute; data: PrototypeData}) {
  ...
  switch (route.screen) {
    ...
    case 'task': return <TaskDetail route={route} project={project} runs={runs}/>;
    case 'protocol': return <Protocol route={route} project={project}/>;
...
export function PrototypeShell({route, data}: {route: PrototypeRoute; data: PrototypeData}) {
  ...
  return <div className="fcp-prototype" style={tokenStyle}><Header route={route}/><div className="fcp-main">{route.screen === 'dashboard' ? <Dashboard route={route} data={data}/> : route.screen === 'projects' ? <Projects route={route} data={data}/> : route.screen === 'agents' ? <Agents route={route} access={access}/> : route.screen === 'agent' ? <AgentDetail route={route} access={access}/> : <ProjectScreen route={route} data={data}/>}</div></div>;
}
