...
function TaskDetail({route, project, runs, csrfToken}: {route: PrototypeRoute; project: ProjectData; runs: RunsData | null; csrfToken: string | null}) {
  ...
  const recordedEvidence = journey?.evidence ?? [];
  const nextRequiredEvidence = (journey?.requiredEvidence ?? []).filter((requirement) => !recordedEvidence.some((entry) => entry.requirement === requirement));
