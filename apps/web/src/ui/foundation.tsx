...
import {WorkspaceLink} from './workspace-link.tsx';
...
export function ProjectListRow({name,status,setup,facts,href,nextAction}:Readonly<{name:string;status:ReactNode;setup:string;facts:readonly string[];href:string;nextAction:string}>){return <article className="fcp-c-project-row"><div className="fcp-c-project-row-title"><WorkspaceLink href={href}>{name}</WorkspaceLink><span>{setup}</span></div><div className="fcp-c-project-row-status">{status}</div><p>{facts.map((fact,index)=><span key={`${fact}-${index}`}>{fact}</span>)}</p><WorkspaceLink className="fcp-c-project-row-action" href={href} aria-label={`${nextAction}: ${name}`}>{nextAction}<ChevronRight aria-hidden="true" size={16}/></WorkspaceLink></article>;}

export type SetupNode=Readonly<{id:string;label:string;state:'complete'|'skipped'|'active'|'pending'|'error'}>;
export function SetupRail({nodes,onSelect}:Readonly<{nodes:readonly SetupNode[];onSelect?:(id:string)=>void}>){const marker=(node:SetupNode,index:number)=>node.state==='complete'?<Check aria-hidden="true" size={14}/>:node.state==='skipped'?'–':index+1;return <ol className="fcp-c-setup-rail" aria-label="Готовность проекта">{nodes.map((node,index)=><li className={`is-${node.state}`} key={node.id}>{onSelect===undefined?<span className="fcp-c-setup-node">{marker(node,index)}<b>{node.label}</b></span>:<button type="button" className="fcp-c-setup-node" disabled={node.state==='pending'} onClick={()=>onSelect(node.id)}>{marker(node,index)}<b>{node.label}</b></button>}{index===nodes.length-1?null:<i aria-hidden="true"/>}</li>)}</ol>;}
