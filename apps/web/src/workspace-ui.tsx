...
function Dashboard({route, projects, portfolio}: {
  ...
  return <><div className="fcp-page-title"><div><h1>Обзор проектов</h1><p>Подтверждённый прогресс каждого доступного проекта — по весу результатов.</p></div><Scope route={route}/></div><section className="fcp-dashboard-progress" aria-label="Прогресс доступных проектов">{projects.length === 0 ? <p className="fcp-empty-line">Нет доступных проектов.</p> : projects.map((project) => {
    ...
    return <Link className="fcp-dashboard-progress-card" href={projectUrl(project.project.slug, 'overview', route.scope)} key={project.project.slug}><header><div><span>{project.project.name}</span><small>Принятый скоп</small></div><Status value={facts?.health ?? 'unknown'}/></header>{progress === null || progress.totalWeight === 0 ? <strong>Не настроено</strong> : <><strong>{progress.acceptedWeight} / {progress.totalWeight}</strong><div className="fcp-dashboard-progress-bar" aria-label={progress.states.map((item) => `${item.label}: ${item.weight}`).join(', ')}>{progress.states.map((item) => <span className={`fcp-scope-${item.key}`} key={item.key} style={{width: `${item.weight / progress.totalWeight * 100}%`}}/>)}</div><ul>{progress.states.map((item) => <li key={item.key} className={`fcp-scope-${item.key}`}><span aria-hidden="true"/>{item.label} {item.weight}</li>)}</ul></>}<dl className="fcp-dashboard-facts"><div><dt>Текущий этап</dt><dd>{project.execution?.selection?.stageName ?? (project.execution?.status === 'completed' ? 'Завершено' : 'Не определён')}</dd></div><div><dt>Ближайший срок</dt><dd>{deadline === null ? 'Не задан' : ruDate(deadline)}</dd></div><div><dt>Риски</dt><dd>{facts === null ? 'Не загружены' : facts.unresolvedRiskCount}</dd></div><div><dt>Главный риск</dt><dd>{topRisk?.reason ?? 'Нет открытого canonical risk'}</dd></div></dl>{topRisk === null ? null : <p className="fcp-dashboard-risk">{topRisk.owner ?? 'Ответственный не назначен'} · {date(topRisk.freshness)}<br/>{topRisk.nextAction}</p>}</Link>;
  })}</section></>;
}
...
function Overview({route, project, runs, portfolio, csrfToken, canManage, hasWriteCapability, canApproveOutcome}: {route: WorkspaceUiRoute; project: ProjectData; runs: RunsData | null; portfolio: PortfolioData | null; csrfToken: string | null; canManage: boolean; hasWriteCapability: boolean; canApproveOutcome: boolean}) {
  ...
  return <><ProjectHeader route={route} project={project}/><ContextTabs label="Разделы обзора" items={[{label: 'Сводка', href: projectUrl(project.project.slug, 'overview', route.scope), active: true}, {label: 'Скоп', href: '#scope', active: false}, {label: 'Риски', href: '#risks', active: false}]}/><ProjectExecutionControls projectId={project.project.id} execution={execution} csrfToken={csrfToken} canManage={canManage} hasWriteCapability={hasWriteCapability} runnerQueueAvailable={runnerActivationEnabled()}/><ScopeBaseline project={project} csrfToken={csrfToken} canApproveOutcome={canApproveOutcome}/><section className="fcp-section" id="risks"><div className="fcp-section-head"><div><h2>Риски проекта</h2><span>Только открытые canonical RiskSignal</span></div></div>{risks.length === 0 ? <p className="fcp-empty-line">Открытых canonical risks нет.</p> : <div className="fcp-list">{risks.map((risk) => <article className="fcp-risk-row" key={risk.id}><Status value={risk.severity}/><div><strong>{risk.reason}</strong><small>{risk.owner ?? 'Ответственный не назначен'} · {date(risk.freshness)}</small><span>{risk.nextAction}</span></div></article>)}</div>}</section><section className="fcp-section" id="attention"><div className="fcp-section-head"><div><h2>Гигиена задач</h2><span>Локальные признаки: блокировка, владелец и следующее действие; это не project risk</span></div><Link href={projectUrl(project.project.slug, 'tasks', route.scope)}>Открыть задачи</Link></div>{attention.length === 0 ? <p className="fcp-empty-line">Нарушений гигиены задач не обнаружено.</p> : <div className="fcp-list">{attention.map((task) => <TaskRow project={project.project.slug} route={route} task={task} projectData={project} key={task.id}/>)}</div>}</section><section className="fcp-section"><div className="fcp-section-head"><h2>Текущая работа</h2><Link href={projectUrl(project.project.slug, 'tasks', route.scope)}>Открыть задачи</Link></div>{active.length === 0 ? <p className="fcp-empty-line">Активные задачи не зафиксированы.</p> : <div className="fcp-list">{active.slice(0, 5).map((task) => <TaskRow project={project.project.slug} route={route} task={task} projectData={project} key={task.id}/>)}</div>}</section>{runs === null ? null : <section className="fcp-section"><div className="fcp-section-head"><h2>Последние запуски</h2><Link href={projectUrl(project.project.slug, 'runs', route.scope)}>Открыть запуски</Link></div>{runs.runs.length === 0 ? <p className="fcp-empty-line">Запуски не зафиксированы.</p> : <div className="fcp-list">{runs.runs.slice(0, 4).map((run) => <RunRow project={project.project.slug} route={route} run={run} key={run.id}/>)}</div>}</section>}</>;
}
...
function ProjectScreen({route, data}: {route: WorkspaceRoute; data: WorkspaceData}) {
  ...
  const canManageExecution = executionMembership?.role === 'project_owner' ||
    executionMembership?.role === 'workspace_owner' || executionOperator?.role === 'workspace_admin' ||
