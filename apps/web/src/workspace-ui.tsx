...
function Overview({route, project, runs, csrfToken, canManage, hasWriteCapability, canApproveOutcome}: {route: WorkspaceUiRoute; project: ProjectData; runs: RunsData | null; csrfToken: string | null; canManage: boolean; hasWriteCapability: boolean; canApproveOutcome: boolean}) {
...
function ProjectScreen({route, data}: {route: WorkspaceRoute; data: WorkspaceData}) {
  ...
  switch (route.screen) {
    ...
    case 'overview': return <Overview route={route} project={project} runs={runs} csrfToken={data.csrfToken ?? null} canManage={canManageExecution ?? false} hasWriteCapability={hasExecutionWriteCapability} canApproveOutcome={executionMembership?.role === 'project_owner' && hasExecutionWriteCapability}/>;
