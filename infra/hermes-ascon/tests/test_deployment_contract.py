...
class DeploymentContractTest(unittest.TestCase):
    def test_project_logs_are_bounded_and_stage_prunes_only_project_images(self):
        ...
        self.assertNotIn("docker builder prune", application_script)
    ...
    def test_derived_image_and_separate_codex_oauth_are_pinned(self):
        ...
        self.assertNotIn("agent-executor-result.schema.json", compose)
    ...
    def test_gateway_uses_native_project_scoped_credentials_without_brokers(self):
        ...
        self.assertIn('gh auth login --hostname github.com --git-protocol https --with-token', entrypoint)
        self.assertIn('gh auth setup-git', entrypoint)
        self.assertIn('unset GH_TOKEN', entrypoint)
        self.assertNotIn('export GH_TOKEN', entrypoint)
    ...
    def test_private_management_and_direct_devops_runtime_are_bounded(self):
        ...
        worker = application_compose.split("  worker:\n", 1)[1].split("\nnetworks:", 1)[0]
        ...
        self.assertIn("- hermes-management", worker)
        self.assertIn("source: hermes-management-username", worker)
        self.assertIn("source: hermes-management-password", worker)
