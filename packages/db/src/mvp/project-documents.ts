...
export const uploadProjectDocument = async (database: Database, input: Readonly<{
  workspaceId: string; projectId: string; actorId: string; category: ProjectDocumentCategory;
  ...
  const mediaType=validateDocument(input);
  ...
  try {
    ...
    const allowed = await client.query(`select 1 from projects p join project_memberships m on m.project_id=p.id
      ...
      for update of p`, [input.projectId, input.workspaceId, input.actorId]);
    ...
    const active = await client.query<{kind: string; sizeBytes: string | number}>(`select distinct on (kind) kind,
      ...
      order by kind,created_at desc,id desc`, [input.projectId]);
    const id = randomUUID();
    const replacedKind = input.category === 'supplemental' ? `${kind(input.category)}:${id}` : kind(input.category);
    const retained = active.rows.filter((row) => row.kind !== replacedKind);
    if (retained.length + 1 > projectDocumentMaxFiles ||
      retained.reduce((total, row) => total + Number(row.sizeBytes), 0) + input.bytes.byteLength > projectDocumentMaxSetBytes) {
    ...
    const inserted = await client.query<{id: string; createdAt: Date}>(`insert into project_source_artifacts
      ...
      returning id,created_at as "createdAt"`, [id,input.projectId,input.actorId,replacedKind,input.name,mediaType,
      sha256,input.bytes,input.provenance]);
    ...
    return {id:row.id,projectId:input.projectId,category:input.category,artifactKind:replacedKind,
      name:input.name,mediaType,
      sha256,sizeBytes:input.bytes.byteLength,provenance:input.provenance,createdAt:row.createdAt.toISOString(),
      downloadPath:`/api/projects/${input.projectId}/documents/${row.id}`};
