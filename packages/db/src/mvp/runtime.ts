...
export const saveProjectExecutionMode = async (database: Database, input: Readonly<{
  ...
  try {
    await client.query('begin');
    await client.query('select pg_advisory_xact_lock(hashtextextended($1,0))', [`project-execution-mode:${input.projectId}`]);
    await client.query('select pg_advisory_xact_lock(hashtextextended($1,0))', [input.idempotencyKey]);
...
export const createApprovalPersistence = (database: Database): Readonly<{
  ...
  targets: {async resolve(input) {
    ...
    const proposal=await database.query<{url:string;version:string}>(`select b.project_url as url,s.sha256 as version
      ...
      where s.project_id=$1 and s.kind in ('project_architecture_proposal_v1','project_tracker_preparation_approval_v1')
        and s.sha256=$2 limit 1`,
  ...
  transaction: {async record(input) {
    ...
    try {
      ...
      if(currentFact===undefined&&['plan','internal_operation'].includes(input.evidence.kind)&&/^[a-f0-9]{64}$/.test(input.evidence.target.id)){
...
export const executeAutonomousPmTransaction=async(database:Database,input:Readonly<{workspaceId:string;
  ...
  routingVersion:string;snapshotVersion:string;modeChangedAt:string}>,submit:()=>Promise<Readonly<{deliveryReference:string}>>):Promise<
  Readonly<{status:'started'|'duplicate'|'busy'|'disabled';deliveryReference:string|null}>>=>{const client=await database.connect();try{
  await client.query('begin');await client.query('select pg_advisory_xact_lock(hashtextextended($1,0))',
    [`project-execution-mode:${input.projectId}`]);
  const mode=await client.query(`select 1 from audit_events a where a.project_id=$1 and a.actor_id=$2 and
    ...
    a.id=(select latest.id from audit_events latest where latest.project_id=$1 and latest.action='project.execution.mode'
      order by latest.occurred_at desc,latest.created_at desc limit 1)`,[input.projectId,input.actorId,input.modeChangedAt]);
  if(mode.rowCount===0){await client.query('rollback');return {status:'disabled',deliveryReference:null};}
  const prior=await client.query<{deliveryReference:string}>(`select result_reference as "deliveryReference"
    from command_receipts where idempotency_key=$1 and command_type='autonomous.pm'`,[input.idempotencyKey]);
  ...
  const active=await client.query(`select 1 from audit_events a where a.project_id=$1 and (
    (a.action='agent.submit' and not exists(select 1 from audit_events t where t.project_id=a.project_id and
      t.correlation_id=a.correlation_id and t.action in ('agent.attempt.completed','agent.attempt.failed'))) or
    (a.action='autonomous.pm' and not exists(select 1 from audit_events t where t.project_id=a.project_id and
      t.correlation_id=a.correlation_id and t.action in ('autonomous.pm.completed','autonomous.pm.failed')))) limit 1`,
  [input.projectId]);if(active.rowCount!==0){await client.query('rollback');return {status:'busy',deliveryReference:null};}
  const delivered=await submit();const occurredAt=new Date().toISOString();
  await client.query(`insert into command_receipts(project_id,actor_id,idempotency_key,command_type,result_reference,occurred_at)
    values($1,$2,$3,'autonomous.pm',$4,$5)`,[input.projectId,input.actorId,input.idempotencyKey,
    delivered.deliveryReference,occurredAt]);
  await client.query(`insert into audit_events(workspace_id,project_id,actor_id,action,target_reference,correlation_id,details,occurred_at)
    values($1,$2,$3,'autonomous.pm',$4,$5,$6,$7)`,[input.workspaceId,input.projectId,input.actorId,
    input.projectId,input.correlationId,JSON.stringify({deliveryReference:delivered.deliveryReference,
      processVersion:input.processVersion,routingVersion:input.routingVersion,snapshotVersion:input.snapshotVersion,
      modeChangedAt:input.modeChangedAt}),occurredAt]);
  await client.query('commit');return {status:'started',deliveryReference:delivered.deliveryReference};
}catch(error){await client.query('rollback');throw error;}finally{client.release();}};

export const listActiveAutonomousPmAttempts=async(database:Database,workspaceId:string,limit=20):Promise<
  readonly AutonomousPmAttempt[]>=>{const result=await database.query<AutonomousPmAttempt>(`select a.workspace_id as "workspaceId",
...
export const hasActiveAgentAttempt=async(database:Database,projectId:string):Promise<boolean>=>{
  const result=await database.query(`select 1 from audit_events a where a.project_id=$1 and a.action='agent.submit'
    and not exists(select 1 from audit_events t where t.project_id=a.project_id and t.correlation_id=a.correlation_id
      and t.action in ('agent.attempt.completed','agent.attempt.failed')) limit 1`,[projectId]);
  return result.rowCount!==0;
};

export const finishAutonomousPmAttempt=async(database:Database,attempt:AutonomousPmAttempt,input:Readonly<{
  status:'completed'|'failed';result:AutonomousPmResult|null;notification:MessengerDeliveryInput|null}>):Promise<
  'recorded'|'duplicate'>=>{const client=await database.connect();try{await client.query('begin');
  await client.query('select pg_advisory_xact_lock(hashtextextended($1,0))',[attempt.correlationId]);
  const prior=await client.query(`select 1 from audit_events where project_id=$1 and correlation_id=$2 and
    action in ('autonomous.pm.completed','autonomous.pm.failed')`,[attempt.projectId,attempt.correlationId]);
  if(prior.rowCount!==0){await client.query('rollback');return 'duplicate';}const occurredAt=new Date().toISOString();
  await client.query(`insert into audit_events(workspace_id,project_id,actor_id,action,target_reference,correlation_id,details,occurred_at)
    values($1,$2,$3,$4,$5,$6,$7,$8)`,[attempt.workspaceId,attempt.projectId,attempt.actorId,
    `autonomous.pm.${input.status}`,attempt.projectId,attempt.correlationId,JSON.stringify({status:input.status,
      deliveryReference:attempt.deliveryReference,...(input.result===null?{}:{result:input.result})}),occurredAt]);
  if(input.notification!==null)await client.query(`insert into outbox_events(project_id,topic,idempotency_key,payload,available_at)
    values($1,'messenger-notification',$2,$3,$4) on conflict(idempotency_key) do nothing`,[attempt.projectId,
    input.notification.idempotencyKey,JSON.stringify({message:input.notification}),occurredAt]);
  await client.query('commit');return 'recorded';}catch(error){await client.query('rollback');throw error;}
  finally{client.release();}};
