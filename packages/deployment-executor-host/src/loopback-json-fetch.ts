...
export const createLoopbackJsonFetch = (): typeof fetch => (async (input, init = {}) => {
  ...
  if (init.method !== 'POST' || (init.body !== undefined && typeof init.body !== 'string')) return fail('request');
  ...
  return new Promise<Response>((resolve, reject) => {
    const request = httpRequest(input, {method: 'POST', headers: {...headers,
      ...
      response.on('end', () => resolve(Object.freeze({status: response.statusCode ?? 0,
        json: async (): Promise<unknown> => JSON.parse(Buffer.concat(chunks).toString('utf8')) as unknown
