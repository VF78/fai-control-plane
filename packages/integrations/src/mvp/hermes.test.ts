...
const signedReceipt = (overrides: Partial<Record<string, string>> = {}) => {
  const unsigned = {
    contract: 'fai.executor-invocation-receipt.v1' as const,
    invocationId: overrides.invocationId ?? 'a'.repeat(64),
    receiptReference: overrides.receiptReference ?? request.correlationId,
    executorId: overrides.executorId ?? 'codex-cli', model: overrides.model ?? 'gpt-5.6-terra',
    effort: (overrides.effort ?? 'medium') as 'medium'|'high', outputSha256: overrides.outputSha256 ?? 'b'.repeat(64),
    completedAt: overrides.completedAt ?? '2026-08-25T10:00:00.000Z'
  };
  const payload = [unsigned.contract, unsigned.invocationId, unsigned.receiptReference, unsigned.executorId,
    unsigned.model, unsigned.effort, unsigned.outputSha256, unsigned.completedAt].join('\n');
  return {...unsigned, signature: sign(null, Buffer.from(payload), keys.privateKey).toString('base64')};
};
...
describe('MVP Hermes adapter', () => {
  it('delivers the neutral role contract and returns opaque evidence', async () => {
    ...
    const adapter = createHermesDeliveryAdapter({endpoint: 'https://agent.example.test/v1/runs',
      ...
      secrets: {resolve: async () => ({value: 'bearer'})}, executorAttestationPublicKey: publicKey, fetch});
  ...
  it('accepts only the bounded executor-result contract and preserves stable deliverable links', async () => {
    const result = {contract: 'fai.agent-executor-result.v1', decision: 'accepted', ...attestation,
      executorReceipt: signedReceipt(), reason: 'Ready for QA',
      evidence: [{kind: 'checks', result: 'Focused tests passed'}],
      deliverables: [{label: 'Review document', url: 'https://example.test/result.docx'}]};
    const adapter = createHermesDeliveryAdapter({endpoint: 'https://hermes.example/v1/runs',
      credentialRef: {id: 'secret', purpose: 'agent_delivery', locator: '/run/secrets/agent'},
      secrets: {resolve: async () => ({value: 'bearer'})}, executorAttestationPublicKey: publicKey, fetch: vi.fn(async () =>
  ...
  it.each([
    {name: 'missing receipt', receipt: undefined, key: publicKey},
    {name: 'altered signed route', receipt: {...signedReceipt(), model: 'gpt-5.6-sol'}, key: publicKey},
    {name: 'wrong signing key', receipt: signedReceipt(), key: generateKeyPairSync('ed25519').publicKey
      .export({type: 'spki', format: 'pem'}).toString()}
  ])('fails closed for $name', async ({receipt, key}) => {
    const result = {contract: 'fai.agent-executor-result.v1', decision: 'accepted', ...attestation,
      ...(receipt === undefined ? {} : {executorReceipt: receipt}), reason: 'done',
      evidence: [{kind: 'checks', result: 'passed'}]};
    const adapter = createHermesDeliveryAdapter({endpoint: 'https://hermes.example/v1/runs',
      credentialRef: {id: 'secret', purpose: 'agent_delivery', locator: '/run/secrets/agent'},
      secrets: {resolve: async () => ({value: 'bearer'})}, executorAttestationPublicKey: key,
  ...
  it('keeps direct-agent results receipt-free', async () => {
  ...
  it('keeps an expired run unobservable instead of treating 404 as failure', async () => {
    const adapter = createHermesDeliveryAdapter({endpoint: 'https://hermes.example/v1/runs',
      credentialRef: {id: 'secret', purpose: 'agent_delivery', locator: '/run/secrets/agent'},
      secrets: {resolve: async () => ({value: 'bearer'})}, executorAttestationPublicKey: publicKey, fetch: vi.fn(async (_input, init) => {
  ...
  it('rejects a non-HTTPS agent endpoint at composition', () => {
    expect(() => createHermesDeliveryAdapter({endpoint: 'http://agent.example.test/v1/runs',
      credentialRef: {id: 'secret', purpose: 'agent', locator: '/run/secrets/agent'},
      secrets: {resolve: async () => ({value: 'bearer'})}, executorAttestationPublicKey: publicKey})).toThrow('agent_endpoint_invalid');
  ...
  it('rejects malformed provider evidence', async () => {
    const adapter = createHermesDeliveryAdapter({endpoint: 'https://agent.example.test/v1/runs',
      credentialRef: {id: 'secret', purpose: 'agent', locator: '/run/secrets/agent'},
      secrets: {resolve: async () => ({value: 'bearer'})}, executorAttestationPublicKey: publicKey,
