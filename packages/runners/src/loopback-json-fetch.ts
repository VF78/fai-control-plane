// Node 22's ESM facade enumerates lazy node:http WebSocket exports and initializes
// undici. Accessing the fixed built-in module loads only the HTTP request API,
// which remains compatible with the runner's required --jitless boundary.
const nodeHttp = process.getBuiltinModule('node:http') as typeof import('node:http') | undefined;
if (nodeHttp === undefined) throw new Error('loopback_json_fetch_http_unavailable');
const httpRequest = nodeHttp.request;

const LOOPBACK_HOSTS = new Set(['127.0.0.1', '[::1]']);
const ALLOWED_HEADERS = new Set(['authorization', 'content-type', 'x-fai-runner-lease-token']);
const MAX_REQUEST_BYTES = 256 * 1024;
const MAX_RESPONSE_BYTES = 256 * 1024;
const TIMEOUT_MS = 15_000;

const fail = (code: string): never => { throw new Error(`loopback_json_fetch_${code}`); };

export const isNumericLoopbackHostname = (hostname: string): boolean => LOOPBACK_HOSTS.has(hostname);

const requestHeaders = (value: HeadersInit | undefined): Record<string, string> => {
  if (value === undefined) return {};
  if (Array.isArray(value) || typeof value !== 'object' || value === null ||
    Object.getPrototypeOf(value) !== Object.prototype) return fail('headers');
  const result: Record<string, string> = {};
  for (const [rawName, rawValue] of Object.entries(value)) {
    const name = rawName.toLowerCase();
    if (!ALLOWED_HEADERS.has(name) || typeof rawValue !== 'string' || rawValue.length > 512 ||
      /[\r\n]/.test(rawValue)) return fail('headers');
    result[name] = rawValue;
  }
  return result;
};

export const createLoopbackJsonFetch = (): typeof fetch => (async (input, init = {}) => {
  if (!(input instanceof URL) || input.protocol !== 'http:' || !isNumericLoopbackHostname(input.hostname) ||
    input.username !== '' || input.password !== '' || input.hash !== '' ||
    Object.keys(init).some((key) => !['method', 'headers', 'body'].includes(key))) return fail('request');
  if (init.method !== 'POST' || (init.body !== undefined && typeof init.body !== 'string')) {
    return fail('request');
  }
  const body = init.body ?? '';
  if (Buffer.byteLength(body) > MAX_REQUEST_BYTES) return fail('request_size');
  const headers = requestHeaders(init.headers);
  return await new Promise<Response>((resolve, reject) => {
    const request = httpRequest(input, {method: 'POST', headers: {
      ...headers,
      'content-length': String(Buffer.byteLength(body))
    }}, (response) => {
      const chunks: Buffer[] = [];
      let size = 0;
      response.on('data', (chunk: Buffer) => {
        size += chunk.byteLength;
        if (size > MAX_RESPONSE_BYTES) {
          response.destroy(new Error('loopback_json_fetch_response_size'));
          return;
        }
        chunks.push(chunk);
      });
      response.on('error', reject);
      response.on('end', () => {
        const payload = Buffer.concat(chunks).toString('utf8');
        resolve(Object.freeze({
          status: response.statusCode ?? 0,
          json: async (): Promise<unknown> => JSON.parse(payload) as unknown
        }) as unknown as Response);
      });
    });
    request.on('error', reject);
    request.setTimeout(TIMEOUT_MS, () => request.destroy(new Error('loopback_json_fetch_timeout')));
    request.end(body);
  });
}) as typeof fetch;
